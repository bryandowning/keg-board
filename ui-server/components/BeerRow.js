...
const BeerRow = ({ beer }) => {
  let srmNumber = beer.srm;
  if (beer.srm < 1) {
    srmNumber = 1;
  } else if (beer.srm > 40) {
    srmNumber = 40;
  }
  const srmBackgroundColor =
    srmNumber !== '' ? srmToRGBLookupTable[Math.round(srmNumber)] : srmToRGBLookupTable[1]; // TODO: right way to check empty string?
  ...
  let calFromAlc = (1881.22 * (beer.fg * (beer.og - beer.fg))) / (1.775 - beer.og);
  let calFromCarbs = 3550.0 * beer.fg * (0.1808 * beer.og + 0.8192 * beer.fg - 1.0004);
